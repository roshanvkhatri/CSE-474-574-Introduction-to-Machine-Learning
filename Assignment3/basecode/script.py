...
def mlrObjFunction(params, *args):
    ...
    # bias term is created and added to the data
    ...
    theta_matrix = (np.exp(dp_weight_vec_X) / theta_sum_matrix)
    ...
    # Calculate graident wrt weight vector
...
def mlrPredict(W, data):
    ...
    dp_weight_vec_X = np.dot(train_data, W)
    theta_matrix_sum = np.sum(np.exp(dp_weight_vec_X))
    posterior = np.exp(dp_weight_vec_X) / theta_matrix_sum
